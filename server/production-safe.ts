// Production-safe server that completely avoids any Vite dependencies
...
function serveStatic(app: express.Express) {
  // Use import.meta.dirname replacement for better compatibility
...
async function registerRoutes(app: express.Express): Promise<Server> {
  ...
  // Essential API routes for production (no external dependencies)
...
(async () => {
  try {
    console.log('Starting production-safe server...');
    ...
    console.error('Failed to start production-safe server:', error);
    process.exit(1);
  }
})();