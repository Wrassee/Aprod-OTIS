// Safe Vite wrapper - Replace direct Vite imports with dynamic imports to prevent bundling issues
...
export async function setupVite(app: Express, server: Server) {
  ...
  try {
    ...
    console.log('Attempting dynamic Vite import...');
    const vite = await import("vite").catch((error) => {
      console.log('Vite not available:', error.message);
      return null;
    });
    
    if (!vite || !vite.createServer || !vite.createLogger) {
    ...
    console.log('Continuing with fallback static serving');
  }
}
